"""ABC coverage evaluation, stage 2: finalize the mechanical passes into
schema-valid PilotClaimResult sets + metrics (mirroring the frozen D4
comparison's ``finalize_all.py`` structure exactly), and verify
claim -> matched evidence -> exact-source provenance for every supported
claim.

Category assignments: after A+B+C the ONLY still-divergent claims are the
four R1 nested-function-closure claims (resolution_failed) and the frozen
out-of-scope entries. The R1 root cause is now precisely known (and is
intervention D's scope, deliberately not implemented here): the real
parser DOES emit the nested-function symbols, but classifies any def
nested inside another def as ``kind="method"``, while the frozen ground
truth (correctly) declares them ``kind="function"`` -- so the harness's
kind-checked StableEntityRef resolution finds no match.
"""
...
_R1_NESTED_DEF_NOTE = (
    "Unchanged by A+B+C (intervention D scope, deliberately not implemented in this round): "
    "the subject is a function nested inside another def. The real parser DOES emit the symbol "
    "(qualified name matches), but classifies every nested def as kind='method', while the frozen "
    "ground truth correctly declares kind='function' -- the harness's kind-checked StableEntityRef "
    "resolution therefore finds no match and the claim cannot be evaluated."
)

R1_OVERRIDES = {
    "R1-TDC-5": ("parser_coverage", _R1_NESTED_DEF_NOTE),
    "R1-TRC-2": ("parser_coverage", _R1_NESTED_DEF_NOTE),
    "R1-FDC-5": ("parser_coverage", _R1_NESTED_DEF_NOTE),
    "R1-FRC-2": ("parser_coverage", _R1_NESTED_DEF_NOTE),
}
...
def _verify_provenance(results, claims_by_id, run) -> list[dict]:
    ...
    for result in results:
        ...
        for relation_id in result.matched_evidence:
            ...
            # (the observed call/declaration site lives in the claimed
            # subject/hop-source). The ground truth's cited location is a
            # proof-of-truth citation, not an exhaustive call-site
            # inventory -- the verifier may legitimately have matched a
            # different genuine occurrence of the same pair (e.g. the same
            # method called twice in one body), so GT-citation match is
            # recorded as an informational flag, not asserted.
...
def main() -> None:
    ...
    for tier, repo_id, revision, analysis_root, claims_path, overrides in REPOS:
        ...
        manifest["comparison_note"] = (
            "ABC coverage evaluation -- NOT pilot-v1 and NOT the frozen D4 comparison; system "
            "checkpoint is this repository's backend with interventions A (subscripted generic "
            "inheritance bases), B (bare `from . import X` submodule binding), and C (guarded "
            "static super() resolution) applied on top of the J3+D4 state."
        )
