"""D coverage evaluation, stage 1 (mechanical pass): run the SAME frozen
R1/R2/R3 ground truth through THIS repository's backend (intervention D --
nested-definition symbol kind + guarded closure-``self`` binding -- applied
on top of the A+B+C state at checkpoint 88a3689) using the byte-identical
frozen pilot harness (``evaluate_repository.py`` from the
``lauras-d4-comparison`` worktree, imported read-only).

Nothing under the frozen pilot/D4-comparison worktrees is written to --
all output goes under this directory (``research/d-coverage-evaluation/``).

Same deliberate wiring difference as the ABC evaluation: the
``syntax_tree_refurbished`` package is pre-imported from THIS repository's
``syntax-tree-refurbished-backend/src`` BEFORE the pilot harness is
imported, so the harness's own ``sys.path`` insertion of its worktree's
backend cannot shadow the system under evaluation. The script asserts
which backend was actually imported and records it in the output.
"""
