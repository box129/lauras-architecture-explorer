...
export default function ArchitectureGraphInspector({ node, onEnter, onExpand, scopeGroupId }: { node: ObservatoryNode | null; onEnter: (node: ObservatoryNode) => void; onExpand: (node: ObservatoryNode) => void; scopeGroupId?: string | null }) {
  ...
  if (!selected || !node) return <aside className="architecture-graph-inspector" aria-label="Architecture graph inspector"><p className="architecture-graph-inspector__eyebrow">Architecture overview</p><h2>{scoped.groups.filter((group) => !group.parent_group_id).reduce((count, group) => count + group.recursive_module_count, 0)} modules</h2><dl><div><dt>Regions</dt><dd>{scoped.groups.filter((group) => !group.parent_group_id).length}</dd></div><div><dt>Aggregate relations</dt><dd>{scoped.aggregate_edges.length}</dd></div></dl><p className="architecture-graph-inspector__copy"><Network size={15} /> Resolved calls, inheritance, and future imports are shown only when recovered deterministically.</p><p className="architecture-graph-inspector__hint">Select a structural region to inspect its relations.</p></aside>;
