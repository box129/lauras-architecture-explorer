...
import LensNode from '../architecture-map/LensNode';
...
function GraphSurface({ data, selectedNode, onSelectNode, onEnterNode, onHoverNode }: { data: ArchitectureGraphResponse; selectedNode: ObservatoryNode | null; onSelectNode: (node: ObservatoryNode | null) => void; onEnterNode: (node: ObservatoryNode, selectOnly?: boolean) => void; onHoverNode: (node: ObservatoryNode) => void }) {
  ...
  const visibleIds = visibleGroupIds(sourceNodes, expandedIds, zoom);
  useEffect(() => { let cancelled = false; const groups = data.groups.filter((group) => visibleIds.has(group.id)); void layoutArchitectureGraphProof(groups, data.aggregate_edges.filter((edge) => visibleIds.has(edge.source_group_id) && visibleIds.has(edge.target_group_id))).then((result) => { if (!cancelled) setLayout({ routed: result.routedEdgeIds, nodes: result.nodes.map((item) => { const source = sourceNodes.find((node) => node.id === item.id)!; const isContainer = sourceNodes.some((node) => node.parentGroupId === item.id); const connected = !selectedNode || source.id === selectedNode.id || sourceEdges.some((edge) => (edge.source === selectedNode.id && edge.target === source.id) || (edge.target === selectedNode.id && edge.source === source.id)); return { id: item.id, type: 'lensNode', position: item.position, parentId: item.parentId, extent: item.parentId ? 'parent' : undefined, className: selectedNode && !connected ? 'architecture-graph-node--dimmed' : undefined, style: { width: item.width, height: item.height }, data: { ...source, isContainer, isExpanded: false, internalRelationCount: internalCounts.get(item.id) ?? 0, isSelected: selectedNode?.id === item.id, expandHint: isContainer ? 'Shift+click to expand or collapse. Double-click to enter.' : undefined }, draggable: false }; }) }); }); return () => { cancelled = true; }; }, [data, expandedIds, internalCounts, selectedNode?.id, sourceEdges, sourceNodes, visibleIds]);
  const visibleEdges = visibleGraphEdges(sourceEdges, zoom).filter((edge) => visibleIds.has(edge.source) && visibleIds.has(edge.target));
  const edges: Edge[] = visibleEdges.map((edge) => ({ id: edge.id, source: edge.source, target: edge.target, type: 'semantic', label: edge.label, markerEnd: { type: 'arrowclosed' }, className: `architecture-graph-edge architecture-graph-edge--${edge.kind}${selectedNode && edge.source !== selectedNode.id && edge.target !== selectedNode.id ? ' architecture-graph-edge--dimmed' : ''}`, data: { ...edge, visualKind: edge.kind === 'inherits' ? 'boundary' : 'dependency', routed: layout.routed.includes(edge.id) } }));
  return <ReactFlow nodes={layout.nodes} edges={edges} nodeTypes={nodeTypes} edgeTypes={edgeTypes} fitView minZoom={0.35} maxZoom={1.8} onNodeClick={(event, node) => { const value = node.data as unknown as ObservatoryNode; if (event.shiftKey && sourceNodes.some((candidate) => candidate.parentGroupId === value.id)) setExpandedIds((current) => { const next = new Set(current); next.has(value.id) ? next.delete(value.id) : next.add(value.id); return next; }); else onSelectNode(value); }} onNodeDoubleClick={(_, node) => onEnterNode(node.data as unknown as ObservatoryNode)} onNodeMouseEnter={(_, node) => onHoverNode(node.data as unknown as ObservatoryNode)} onPaneClick={() => onSelectNode(null)}><Background gap={24} /><Controls showInteractive={false} /><MiniMap pannable zoomable /></ReactFlow>;
}
